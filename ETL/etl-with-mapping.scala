...
val EVENT_RAW_TABLE_NAME = "events";
...
//load the event data into database 
event_logs_df.write.mode("append").jdbc(url, EVENT_RAW_TABLE_NAME, prop);

// COMMAND ----------

//load the alarm-incident mapping into database 
mapping4
	.write.mode("append")
